...
class View {
    ...
    private function getCachedImage($file)
    {
        ...
        $compiler = new Compiler(ExtensionManager::getRegisteredModifiers());
    ...
    public function renderComponent($controller, $action, $params = array())
    {
        ...
        $bootstrap->run($controller, $action, true);
    ...
    {
        $callable = ExtensionManager::getMethod($methodName);
        
        if (is_callable($callable)) {
            return $callable(...$arguments);
        } else {
            throw new RenderException('Invalid view method ' . $methodName);
        }
    }
}
